...
if __name__ == "__main__":
    ...
    # 只测序列长度 350 的数据集
    SEQ_LEN = 350
    NUM_GROUPS = 5      # real固定数量50×350_group1~5.fasta
    NUM_SEQS = 50

    # 参数列表：可以按需要自己改
    BEAM_LIST = [10, 15, 20, 30]      # 束宽
    W_LIST = [10, 15, 20, 25, 30]        # 窗口长度

    stride = 40          # 三元组训练时的滑动步长（保持不变即可）
    MAX_ITER = 600       # 搜索最大迭代次数

    # 结果输出目录和文件
    out_dir = "results_350_sensitivity"
    os.makedirs(out_dir, exist_ok=True)
    summary_file = os.path.join(out_dir, "summary_350.csv")

    # 写入表头：方便后续用 Excel / Python 画图
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("seq_len,W,beam_width,group_id,mlcs_length,total_time\n")

    print("=== 开始束宽 × 窗口长度敏感性实验（SEQ_LEN=350） ===")

    for W in W_LIST:
        for beam_width in BEAM_LIST:
            print("\n" + "=" * 80)
            print(f"[参数组合] W = {W}, beam_width = {beam_width}")
            print("=" * 80)
            ...
            for gid in range(1, NUM_GROUPS + 1):
                file_path = f"random固定数量50×{SEQ_LEN}_group{gid}.fasta"
                if not os.path.exists(file_path):
                    print(f"[警告] 数据集不存在: {file_path}，跳过该 group。")
                    continue

                print(f"\n[运行] SEQ_LEN={SEQ_LEN}, group={gid}, W={W}, beam={beam_width}")
                group_start = time.time()

                # 1. 读取序列
                seqs = read_sequences_from_file(file_path)
                print(f"  加载序列数: {len(seqs)} 条")

                # 2. 初始化并训练 Transformer（当前窗口长度 W）
                model = TransformerRDEModel(vocab_size=VOCAB_SIZE)
                model = model.to(device)
                ...
                # 3. 预计算所有窗口嵌入
                all_embeddings = precompute_all_embeddings(
                    model,
                    seqs,
                    W=W,
                    device=device
                ...
                # 4. 带 RDE 的束搜索 MLCS
                mlcs, mlcs_len, ub_best = greedy_mlcs_with_rde(
                    seqs,
                    model,
                    all_embeddings,
                ...
                print(f"  [结果] MLCS 长度 = {mlcs_len}, 总时间 = {group_time:.2f} s, 上界 = {ub_best}")

                all_lengths.append(mlcs_len)
                all_times.append(group_time)

                
                with open(summary_file, "a", encoding="utf-8") as f:
                    f.write(f"{SEQ_LEN},{W},{beam_width},{gid},{mlcs_len},{group_time:.4f}\n")

            # 对当前 (W, beam_width) 做平均
            if all_lengths:
                avg_len = sum(all_lengths) / len(all_lengths)
                avg_time = sum(all_times) / len(all_times)
                print(f"\n[汇总] SEQ_LEN={SEQ_LEN}, W={W}, beam={beam_width}")
                print(f"  平均 MLCS 长度 = {avg_len:.2f}")
                print(f"  平均总时间     = {avg_time:.2f} s")
            else:
                print(f"\n[警告] W={W}, beam={beam_width} 下没有有效结果（可能所有 group 数据集都缺失）。")

    print("\n=== 敏感性实验完成，结果已写入 results_350_sensitivity/summary_350.csv ===\n")
